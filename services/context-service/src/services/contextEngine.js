...
const { SUPPORTED_LANGUAGES, FRAMEWORKS, FILE_EXTENSIONS } = require('@lpa/shared');
...
class ContextEngine {
  ...
  async analyzeFiles(files) {
    try {
      ...
      const summary = this.generateSummary(fileAnalysis, languageInfo, projectType);
      const suggestions = this.generateSuggestions(fileAnalysis, languageInfo, projectType);
  ...
  analyzeFileStructure(files) {
    ...
    return {
      ...
      structure,
      depth: Math.max(...Array.from(directories).map(d => d.split(path.sep).length), 1)
    };
  }
  ...
  detectFrameworkIndicators(file, indicators) {
    ...
    if (fileName === 'package.json' && file.content) {
      try {
        ...
        if (deps['@angular/core']) indicators.angular = (indicators.angular || 0) + 10;
        ...
        if (deps['react-router']) indicators.react = (indicators.react || 0) + 3;
    ...
    if (fileName === 'requirements.txt' && file.content) {
      ...
      lines.forEach(line => {
        ...
        if (dep.includes('django')) indicators.django = (indicators.django || 0) + 8;
        if (dep.includes('flask')) indicators.flask = (indicators.flask || 0) + 8;
        if (dep.includes('fastapi')) indicators.fastapi = (indicators.fastapi || 0) + 8;
      });
    }
    
    // File structure indicators
    if (fileName === 'next.config.js') indicators.nextjs = (indicators.nextjs || 0) + 8;
    if (fileName === 'angular.json') indicators.angular = (indicators.angular || 0) + 8;
    if (fileName === 'vue.config.js') indicators.vue = (indicators.vue || 0) + 8;
    if (fileName === 'svelte.config.js') indicators.svelte = (indicators.svelte || 0) + 8;
    if (fileName === 'nuxt.config.js') indicators.nuxtjs = (indicators.nuxtjs || 0) + 8;
    
    // Directory structure indicators
    if (filePath.includes('/src/components/')) indicators.react = (indicators.react || 0) + 2;
    if (filePath.includes('/pages/')) indicators.nextjs = (indicators.nextjs || 0) + 2;
    if (filePath.includes('/app/')) indicators.rails = (indicators.rails || 0) + 2;
    
    // File content indicators (if available)
    if (file.content) {
      const content = file.content.toLowerCase();
      if (content.includes('import react')) indicators.react = (indicators.react || 0) + 3;
      if (content.includes('from django')) indicators.django = (indicators.django || 0) + 3;
      if (content.includes('from flask')) indicators.flask = (indicators.flask || 0) + 3;
      if (content.includes('express()')) indicators.express = (indicators.express || 0) + 3;
    }
  }
  ...
  detectProjectType(files, dependencies) {
    ...
    dependencies.forEach(dep => {
      ...
      if (['express', 'fastapi', 'django', 'flask', 'spring'].some(fw => name.includes(fw))) {
        indicators.backend = (indicators.backend || 0) + 3;
      }
      
      if (['jest', 'mocha', 'pytest', 'junit'].some(test => name.includes(test))) {
    ...
    if (indicators.frontend && indicators.backend && 
  ...
  rankFileRelevance(files, focusArea) {
    return files.map(file => {
      ...
      if (fileName.includes('component') || fileName.includes('page')) score += 5;
      if (fileName.includes('api') || fileName.includes('route')) score += 5;
      if (fileName.includes('util') || fileName.includes('helper')) score += 3;
      ...
      if (['index', 'main', 'app'].some(name => fileName.includes(name))) score += 3;
      
      // Reduce score for generated/build files
      if (filePath.includes('node_modules') || filePath.includes('dist') || 
          filePath.includes('build') || filePath.includes('.git')) {
        score -= 20;
      }
  ...
  generateSuggestions(fileAnalysis, languageInfo, projectType) {
    ...
    if (languageInfo.framework === 'react') {
      if (!fileAnalysis.structure.src) {
        suggestions.push('Consider organizing your React components in a src/ directory');
      }
      if (!fileAnalysis.structure.components && !fileAnalysis.structure.src?.subdirs?.components) {
        suggestions.push('Create a components/ directory to organize your React components');
      }
    }
    
    // General project structure suggestions
    if (fileAnalysis.totalFiles > 20 && fileAnalysis.depth < 2) {
      suggestions.push('Consider organizing your files into subdirectories for better structure');
    }
    ...
    const hasReadme = Object.values(fileAnalysis.structure).some(dir => 
      dir.files && dir.files.some(f => f.name.toLowerCase().includes('readme'))
    );
